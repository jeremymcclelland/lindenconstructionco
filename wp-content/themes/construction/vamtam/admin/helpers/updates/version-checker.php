...
class Version_Checker {
	...
	private function update_api_request( $update_cache ) {
		...
		$raw_response = wp_remote_post( $this->update_api_url, array(
			'body' => array(
				'version'      => isset( $update_cache->checked[ $theme_name ] ) ? $update_cache->checked[ $theme_name ] : WpvFramework::get_version(),
	...
	public function check_license() {
		...
		if ( ! is_wp_error( $raw_response ) ) {
			if ( $raw_response['response']['code'] >= 200 && $raw_response['response']['code'] < 300  ) {
				...
				_e( 'Valid Purchase Key', 'construction' );
				echo '</span>'; // xss ok
			} else {
				echo '<span style="color: red">'; // xss ok
				_e( 'Incorrect Purchase Key', 'construction' );
				echo '</span>'; // xss ok
			}
		} else {
			echo '<span style="color: red">'; // xss ok
			_e( 'Cannot validate Purchase Key. Please try again later. If the problem persists your server might not have the curl PHP extension enabled.', 'construction' );
			echo '</span>'; // xss ok
		}
