...
(function($, undefined) {
	...
	var ED = {
		...
		get_editor_html: function(refresh) {
			...
			content = content.replace( /<span.*?class="mce_SELRES_[^<]*<\/span>/g, '' );
			...
			accordion: {
				init: function() {
								...
								return '<div class="wpv_accordion">'+
										'<div>\
											<h3 class="title-wrapper clearfix">\
												...
												<a class="accordion-background-selector" data-background-image="" title="Change Pane Background">' + window.WPVED_LANG.image_icon + '</a>\
											</h3>\
					...
					$('body').on('click', '.accordion-background-selector', function( e ) {
						var self     = $( this );
						var selected = self.attr( 'data-background-image' ).split( '/uploads/' )[1];
						...
						file_frame.on( 'select', function() {
							self.attr( 'data-background-image', file_frame.state().get('selection').first().attributes.url );
						});

						file_frame.on( 'close', function() {
							if ( file_frame.state().get('selection').length === 0 ) {
								self.attr( 'data-background-image', '' );
							}
						});
						...
						if ( selected ) {
							...
							query.more().done( function() {
								file_frame.on( 'open', function() {
									...
									file_frame.state().get('library').add( query.models );
									file_frame.state().get('selection').reset( query.models );
								} );
